...
$$ \text{rotation\_matrix} = \begin{bmatrix} \cos(\theta) & -\sin(\theta) & -\sin(\theta) \cdot center_x + (1 - \cos(\theta)) \cdot center_y \\ \sin(\theta) & \cos(\theta) & \cos(\theta) \cdot center_x + \sin(\theta) \cdot center_y - center_y \end{bmatrix} $$
